...
const { nameValidator, emailValidator } = require('../helpers/usersValidators')
...
class UserController {
    ...
    static async updateById(req, res) {
        ...
        try {
            ...
            if(!nome && !email) return res.status(400).send({error:"Nenhum campo encontrado"})
            
            if(nome){
                const isValidName = nameValidator(nome)
                if (isValidName.error) return res.status(400).send(isValidName)
                const nomeValidado = isValidName.nome
                user.nome = nomeValidado
            }

            if(email){
                const isValidEmail = emailValidator(email)
                if (isValidEmail.error) return res.status(400).send(isValidEmail)
                const emailValidado = isValidEmail.email
                user.email = emailValidado
            }
